...
void physics (void)
{
	float angle = myGun->GetAngle() * RADTODEG;
	float inverseAngle;
	if( angle <= 180 )
		inverseAngle = (180 - angle)*DEGTORAD;
	else if( angle <= 270 )
		inverseAngle = (360 - angle - 180)*DEGTORAD;
	else
		inverseAngle = 0;
	...
	else
	{
		...
		if (keys[XK_Up] == 1 || keys[XK_w])
		{
			...
			if(angle < 90.0f && angle > -87.0f)
				myGun->SetTransform(myGun->GetPosition(), myGun->GetAngle() - 3*DEGTORAD);
		...
		{
			if( angle < 90.0f )
				myGun->SetTransform(myGun->GetPosition(), inverseAngle);
			//if(vel.x <= 50)
		...
		if (keys[XK_x] || keys[XK_slash] || keys[XK_z] || keys[XK_period])
		{
			...
			if (timer < 1)
			{
				...
				{
					//Log("left portal\n");
					addRect(M2P*myPlayer->GetPosition().x + player_direction * 61, M2P*myPlayer->GetPosition().y, 5, 5, 0.0f, 0.0f, 1, (char *)"bullet left"); // make a bullet
					//moveBullet(p);
				}
				else
				{
...
void drawPlayer()
{
	b2Vec2 pos = myPlayer->GetPosition();
	glColor3f(0,1,0);
	glPushMatrix();
	glTranslatef(pos.x*M2P, pos.y*M2P, 0);
	glRotatef(myPlayer->GetAngle()*180.0/M_PI, 0, 0, 1);
	b2Vec2 points[4];
	b2Fixture * tmp = myPlayer->GetFixtureList();
	while (tmp)
...
void drawGun()
{
	b2Vec2 pos = myGun->GetPosition();
	glColor3f(1, 0, 0);
	glPushMatrix();
	glTranslatef(pos.x*M2P, pos.y*M2P, 0);
	glRotatef(myGun->GetAngle()*180.0/M_PI, 0, 0, 1);
	b2Vec2 points[4];
	b2Fixture * tmp = myGun->GetFixtureList();
	while (tmp)
	{
		for(int i=0; i < 4; i++)
			points[i] = ((b2PolygonShape*)tmp->GetShape())->GetVertex(i);
